...
const TaskList = ({ title, tasks = [], onToggle, onDelete, onUpdate, onDeleteMany }) => {
  ...
  // Tính số ô trống còn thiếu để đủ 5 ô
  ...
  return (
    <div className='flex min-h-[580px] flex-col rounded-[28px] border border-slate-200/80 bg-white/80 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur-sm dark:border-slate-800 dark:bg-slate-900/70'>
      {/* Header */}
      ...
      <div className='flex flex-1 flex-col space-y-3'>
        {tasks.length === 0 ? (
          <div className='flex flex-1 items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950/40 dark:text-slate-300'>
            {t.noTasks}
          </div>
        ) : (
          <>
            {/* Render danh sách task hiện có */}
            {tasks.map((task) => {
              ...
              return (
                <div
                  ...
                  className={`flex h-[76px] items-center justify-between gap-4 rounded-2xl border p-4 transition-colors ${
            ...
            {/* Render các slot trống placeholder đủ cho 5 items */}
            {Array.from({ length: emptySlotsCount }).map((_, index) => (
              <div
                key={`empty-slot-${index}`}
                className='flex h-[76px] items-center rounded-2xl border border-dashed border-slate-200/80 bg-slate-50/40 p-4 opacity-50 dark:border-slate-800/80 dark:bg-slate-950/20'
              />
            ))}
          </>
        )}
